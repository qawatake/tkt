...
// 日本語の文字化けバグの再現テスト
func TestJapaneseCharacterEncoding(t *testing.T) {
	t.Parallel()
	...
	cases := []struct {
		...
		{
			name:     "escaped square brackets with japanese text",
			input:    `\[これはいけるのでしょうか\] ふがふが`,
			expected: `[これはいけるのでしょうか] ふがふが` + "\n",
		},
		{
			name:     "escaped parentheses with japanese text",
			input:    `\(これは\) いけるのか？`,
			expected: `(これは) いけるのか？` + "\n",
		},
		{
			name:     "plain japanese text",
			input:    `こんにちは世界`,
			expected: `こんにちは世界` + "\n",
		},
		{
			name:     "mixed japanese and english with escapes",
			input:    `\[Japanese\] テスト`,
			expected: `[Japanese] テスト` + "\n",
		},
	}
